...
## Conventions
- New data lives in a module: `models/*.ts` (`model.define`), `service.ts` (extends `MedusaService`), `index.ts` (`Module(...)`), registered in `medusa-config.ts` under `modules`. Then generate and run the migration. Skill: `/medusa-module` (Claude Code) or `$medusa-module` (Codex).
