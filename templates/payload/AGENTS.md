...
## Conventions
- One collection per file exporting a `CollectionConfig` (`slug`, `fields`, plus `access`, `hooks`, `admin` only when needed), registered under `collections` in `payload.config.ts`. Skill: `/payload-collection` or `$payload-collection`.
